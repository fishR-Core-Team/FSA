...
#'period and the survival rate and number of additional individuals added to
#'the population between possible sample periods.  This method assumes that the
#'population is open.
#'
#'If \code{mb.top} contains an object from the \code{capHistSum} function then
#'\code{mb.bot} can be left missing or will be ignored.  In this case, the
#'function will extract the needed data from the \code{methodB.top} and
#'\code{methodB.bot} portions of the \code{CapHist} class object.
#'
#'Seber's (1965) modification to the traditional Jolly-Seber abundance and
#'mortality estimates are used.
#'
#'Manly (1984) did not provide details for computing a confidence interval for
#'the B parameter.  Thus, confidence intervals for this parameter are not
#'included in the output when \code{ci.type="Manly"} is used.  Also, Manly's
#'method does not compute standard errors; therefore, no standard errors are
#'returned when \code{ci.type="Manly"} is used.
#'
#'If \code{type="observables"} in the \code{summary} function then only items
#'that can be observed from the data will be returned -- n, m, R, r, and z.  If
#'\code{type="estimates"} in the \code{summary} function then only items that
#'are estimated from observables will be returned -- M, N, phi, B, and their
#'associated standard errors.
#'
#'The level of confidence is not set in the \code{confint} function, in
#'contrast to most \code{confint} functions.  Rather the confidence level is
#'set in the main \code{mrOpen} function.
#'
#'@aliases mrOpen summary.mrOpen confint.mrOpen
#'@param mb.top A matrix that contains the \dQuote{top} of the Method B table
#'(i.e., a contingency table of capture sample (columns) and last seen sample
#'(rows)) or an object of class \code{CapHist} from the \code{capHistSum}
#'function.  See details.
#'@param mb.bot A data frame that contains the \dQuote{bottom} of the Method B
#'table (i.e., the number of marked fish in the sample (\code{m}), the number
...
#'52:249-259.
#'
#'Seber, G.A.F. 1982. The Estimation of Animal Abundance. Edward Arnold, second
#'edition.
#'
#'Leslie, P.H. and D. Chitty. 1951. The estimation of population parameters
#'from data obtained by means of the capture-recapture method. I. The maximum
#'likelihood equations for estimating the death-rate. Biometrika, 38:269-292.
#'
#'Manly, B.F.J. Obtaining confidence limits on parameters of the Jolly-Seber
#'model for capture-recapture data. Biometrics, 40:749-758.
#'
#'Pollock, K.H., J.D. Nichols, C. Brownie, and J.E. Hines. 1991. Statistical
...
mrOpen <- function(mb.top,mb.bot=NULL,ci.type=c("Jolly","Manly"),conf.level=0.95,phi.type=c("SD","SE")) {
  ...
  # Manly N.CI, phi.CI NOT TESTED
    ...
    est.N <- function(df,type,conf.level) {
      df$N <- (df$n+1)*df$M/(df$m+1)                                            # Estimate population
      if (any(df$N<df$n,na.rm=TRUE)) {
       ...
       switch(type,
         Jolly=,J=,j={ # Jolly large-sample method (from Begon, same as in Pollock et al. 1990)
         ...
         Manly=,M=,m={ # Manly arbitrary method
           p <- df$n/df$N
    ...
    est.phi <- function(df,k,type,conf.level,phi.type) {
       df$M[1] <- 0                                               # By definition, allows estimate of first sample survival
       df1 <- df[-k,]                                             # Current time vector w/o last row to match size of future time vector
       df2 <- df[-1,]                                             # Future time vector w/o first row to match size of current time vector
       phi <- df2$M/(df1$M-df1$m+df1$R)                           # Estimate of phi
       switch(type,
         Jolly=,J=,j={ # Jolly large-sample method (from Begon, same as in Pollock et al. 1990)
  ...
  k <- dim(mb.bot)[2]                                                # Number of samples
  if (k<=2) stop("The Jolly-Seber method requires more than 2 samples.\n",call.=FALSE)
    else {
      df <- data.frame(t(mb.bot)[,-2])                                 # Transpose method B bottom portion, delete u column, make a data.frame
      df$r <- apply(mb.top,1,sum,na.rm=TRUE)                           # Future catches of a sample
      df$r[k] <- NA
      df$z <- rep(NA,k)                                                # Initialize vector for loop
      for (i in 2:(k-1)) {
        df$z[i] <- sum(mb.top[1:(i-1),(i+1):k],na.rm=TRUE)             # Z is sum of matrix with smaller rows and larger columns
      }
      df$M <- df$m+(df$R+1)*df$z/(df$r+1)                              # Estimate marked fish
      df$M.se <- sqrt((df$M-df$m)*(df$M-df$m+df$R)*((1/df$r)-1/df$R))  # Standard error of marked fish
      df <- est.N(df,ci.type,conf.level)                                       # Estimate population sizes with CIs
      df <- est.phi(df,k,ci.type,conf.level,phi.type)                          # Estimate survival rates wtih CIs
      df <- est.B(df,k,ci.type,conf.level)                                     # Estimate additions to the population
      switch(ci.type,
        Jolly=,J=,j={
