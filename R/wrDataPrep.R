...
wrDataPrep <- function(formula,data,units=c("metric","English"),remove.submin=FALSE,
  ...
  cw <- cl[1]
  cl <- cl[2]
  # get the Ws literature values data frame  
  ...
  # which species in the data frame are and are not found in the df list
  areLogical <- specs %in% levels(WSlit$species)
  noWS <- specs[which(!areLogical)]
  if (length(noWS)>0 & remove.other) warning("The following species do not have known standard weight equations:\n ",paste(noWS,collapse=", "),call.=FALSE)
  ...
  for (i in 1:length(specs)) {
    ...
    idata <- data[data[,cspp]==specs[i],]
    # add Ws and Wr columns
    idata <- wrAdd(as.formula(paste(cw,"~",cl)),data=idata,species=specs[i],
                   units=units,remove.submin=remove.submin,wsname=wsname,wrname=wrname)
    # bind current species to the new data frame being created
    ndata <- rbind(ndata,idata)
  }
  # add back in "other" fish if asked not to remove
  if (!remove.other) {
    odata <- data[data[,cspp] %in% noWS,]
    odata[,wsname] <- odata[,wrname] <- NA
    ndata <- rbind(ndata,odata)
  }
  # make sure the species name variables are factored
  ndata[,cspp] <- factor(ndata[,cspp])
