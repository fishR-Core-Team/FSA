...
#'
#'Computes various diet overlap indices between two species from summarized (i.e.,
#'not individual) diet data.
#'
...
#'@param diet1 A numerical vector of \sQuote{amount} (count or biomass) of prey
#'items for the first predator.  Items should be in the same order as amounts in
...
#'that consumed each prey item.  Items should be in same order as amounts in 
#'\code{diet1} and categories in \code{prey}.
#'@param num2 A numerical vector of the number of individuals of the second predator
#'that consumed each prey item.  Items should be in same order as amounts in 
#'\code{diet1} and categories in \code{prey}.
...
#'Memoirs of the Faculty of Science of Kyusha University.  Series in Evolutionary Biology 3:65-80.
#'
#'Pianka R.D. 1973.  The structure of lizard communities. Annual Review of Ecology
...
#'@S3method print dietOverlap
print.dietOverlap <- function(x,...) { print(x$doi,...) }

#'@rdname dietOverlap
#'@method summary dietOverlap
#'@S3method summary dietOverlap
