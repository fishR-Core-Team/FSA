#'Associated S3 methods for bootCase from car.
#'
#'Provides S3 methods to construct non-parametric bootstrap confidence intervals,
#'hypothesis tests, and plots of the parameter estimates for a \code{bootCase()}
#'object (from the \pkg{car} package).
#'
#'\code{confint} finds the two quantiles that have the (1-\code{conf.level})/2
#'proportion of bootstrapped parameter estimates below and above.  This is an
...
#'parameter estimates in that are extreme of the null hypothesized parameter value
#'(\code{bo}).  In the two-tailed alternative, the p-value is twice the smallest
#'of the proportion of bootstrapped parameter \code{bo}.
#'
...
#'that correspond to the approximate confidence interval if \code{object} is a
#'matrix.  If \code{object} is a vector then \code{confint} will return a
#'vector with the two quantiles that correspond to the approximate confidence interval.
#'
#'\code{htest} returns a two-column matrix with the first column containing the
#'hypothesized value sent to this function and the second column containing the
...
#'
#'\code{predict} applies a user-supplied function to each row of \code{object}
#'and then finds the median and the two quantiles that have the proportion
#'(1-\code{conf.level})/2 of the bootstrapped predictions below and above.
...
#'@rdname bootCase
#'@method predict bootCase
#'@S3method predict bootCase
