...
#' @param psdname A string that contains the name for the new length category variable (defaults to \dQuote{PSDcat}).
...
#' @param remove.substock A logical that indicates whether to exclude individual fish with length measurements that are less than the \dQuote{stock} size for that species.  See details.
#' @param remove.other A logical that indicates whether to exclude all individuals of a fish species for which the five-cell length categories do not exist in \code{data(PSDlit)}.  See details.
