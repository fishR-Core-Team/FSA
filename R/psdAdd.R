...
#' 
#' @seealso \code{\link{psdVal}}, \code{\link{psdCalc}}, \code{\link{psdPlot}}, \code{\link{PSDlit}}, \code{\link{tictactoe}}, \code{\link{lencat}}, \code{\link{rcumsum}}, \code{\link{recodeSpecies}}, and \code{\link{wrAdd}}.
#'
...
#'
#' @export
psdAdd <- function(data,formula,units=c("mm","cm","in"),use.names=TRUE) {
  ...
  tmp <- iHndlFormula(formula,data,expNumR=1,expNumE=1,expNumENums=0,expNumEFacts=1)
  if (tmp$vnum!=2) stop("'formula' must have one variable on the left-hand-side\n and one variable on the right-hand-side.",call.=FALSE)
  if (!tmp$metExpNumR) stop("'formula' must have a left-hand-side with one and only one variable.",call.=FALSE)
  if (tmp$Rclass!="numeric") stop("Variable on left-hand-side of 'formula' is not numeric (thus, not lengths).",call.=FALSE)
  if (!tmp$metExpNumE) stop("'formula' must have a right-hand-side with one and only one variable.",call.=FALSE)
  if (!tmp$metExpNumEFacts) stop("'formula' must have one and only one factor variable (species) on right-hand-side.",call.=FALSE)
  ## modify data.frame
  # reduce to length and species data (in that order)
  data <- tmp$mf[,c(tmp$Rpos,tmp$EFactPos)]
  # add rownums variable to data.frame so that it can be sorted
  # back to original order at the end
  data$rownums <- 1:nrow(data)
  # add PSD variable with all NAs
  data$PSD <- rep(NA,nrow(data))
  
  ## get list of species (2ndd column)
  specs <- levels(factor(data[,2]))
  ...
  for (i in 1:length(specs)) {
    ...
    if (specs[i] %in% levels(PSDlit$species)) {
      ...
      tmpdf$PSD <- lencat(tmpdf[,tmp$Rname],breaks=glhse,use.names=use.names)
    }
