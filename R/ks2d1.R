#'Computes the D test statistic for the one-sample two-dimensional
...
#'@param pch A numeric that indicates the character to be used when plotting
#'the results.
...
#'Returned only if \code{KSp=TRUE}.  NOT YET IMPLEMENTED.
#'\item n1 Sample size.
#'\item max1 Maximum D .
...
#'}
#'
#'The \code{plot} function return a scatterplot of the data with the point or 
#'points that resulted in the maximum D value.  The \code{print} function prints
...
ks2d1 <- function(x1,y1,justD=FALSE,KSp=FALSE,divbylen=TRUE) {
  ...
  if (justD) D
    ...
    if (!KSp) res <- list(D=D,n1=n1,where1=where1,divbylen=divbylen,KSp=KSp,x1=x1,y1=y1)
      ...
      pval <- 1-pkolmogorov1x(D,n1)  ## Not sure of this
