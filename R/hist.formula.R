...
#'only a single histogram of the quantitative variable will be produced.  This allows
#'\code{hist.formula()} to be used similarly to \code{hist()} but with a
#'\code{data=} argument.
#'
#'The function produces a single (but see below) graphic that consists of a
#'grid on which the separate histograms are printed.  The rows and columns of
#'this grid are determined to construct a plot that is as square as possible.
#'However, the rows and columns can be set by the user with the \code{nrow=}
#'and \code{ncol=} arguments.  If the product of the number of rows and number
#'of columns set by the user is less than the total number of histograms to be
#'constructed then multiple pages of histograms will be produced (each
#'requiring the user to click on the graph to go to the next graph).  The
#'x-axis of each separate histogram will be labeled identically.  The default
#'x-axis label is the name of the quantitative variable.  This can be changed
#'by the user with the \code{xlab=} argument.
#'
#'By default each histogram is labeled with a main title that consists of the
#'levels formed by the right-hand-side of the formula.  If a string is given in
#'\code{pre.main=} then that string will be appended with the names of the
#'level formed by the right-hand-side of the formula.  If \code{pre.main=NULL}
#'then NO main title will be printed.
#'
#'I changed the \code{right=} argument from that used in the base \code{hist()}
...
#'See details.
#'@param xlab A character label for the x-axis.  Defaults to name of
...
#'\code{boxplot(quantitative~factor)}.  Other methods use functions in
#'\pkg{Lattice} and \pkg{ggplots2} but these packages have some learning
#'\sQuote{overhead} for newbie students.  The formula notation, however, is a
#'common way in R to tell R to separate a quantitative variable by the levels
#'of a factor.  Thus, this function adds code for formulas to the generic
#'\code{hist} function.  This allows newbie students to use a common notation
#'(i.e., formula) to easily create multiple histograms of a quantitative
#'variable separated by the levels of a factor.
#'@note This function is largely a modification of the code provided by Marc
#'Schwartz on the R-help mailing list on 1Jun07.
#'@seealso \code{hist}, \code{multhist} in \pkg{plotrix}.
#'@keywords hplot
#'@examples
#'## Using the defaults
