...
#'
#'The age-agreement table with a Bowker's Test of Symmetry, following Hoenig et al.
#'(1995), is constructed with \code{what="symmetry"} (the default) in \code{summary}.
#'The null hypothesis for this test is that the agreement table is symmetric.  The
#'agreement table can be flipped by including \code{flip.table=TRUE}.
#'
...
#'
#'@seealso \code{\link{agePrecision}}
#'
...
#'summary(ab1,what="symmetry",zero.print="0")
...
plot.ageBias <- function(x,what=c("bias","sunflower"),difference=FALSE,
  ...
  else asunflowerplot(x,difference,xlab,ifelse(!difference,ylab,paste(ylab,"-",xlab)),xlim,ylim,lwd.ref,lty.ref,col.ref,...)
}

#'@rdname ageBias
#'@method summary ageBias
#'@S3method summary ageBias
summary.ageBias <- function(object,what=c("symmetry","bias","diff.bias"),
  ...
  if (what=="symmetry") {
    ...
    diag(at) <- NA
    # Find the values on the lower- and upper- triangels
    lo <- up <- at
    lo[upper.tri(lo)] <- NA
    up[lower.tri(up)] <- NA
    # Chi-sq parts (Hoenig's eq 3)
