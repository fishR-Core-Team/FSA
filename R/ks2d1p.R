...
#'one-sample two-dimensional Kolmogrov-Smirnov test.
#'
#'A permutation test to compute a p-value for the D test statistic for the
#'one-sample two-dimensional Kolmogrov-Smirnov test.
#'
...
ks2d1p <- function(object,B=100) {
  resampD <- function(object) {
    x1 <- object[["x1"]][sample(object[["n1"]],replace=FALSE)] # randomize the x's
    y1 <- object[["y1"]][sample(object[["n1"]],replace=FALSE)] # randomize the y's ... not sure this is necessary
    ks2d1(x1,y1,justD=TRUE,divbylen=object[["divbylen"]])      # send selected points to ks2d1 to compute D
