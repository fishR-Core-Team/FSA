...
For example, `vb()` can now be used to predict the mean length for age-2 fish assuming $L_\infty=450$, $K=0.3$, and $t_0=-0.5$.

```{r}
vb1(2,Linf=450,K=0.3,t0=-0.5)
```

The default result from `makeGrowthFun()` creates a function where the three parameters can be given to the first parameter name as a numeric vector with values for all three parameters. For example, the mean length at age-2 can be predicted with the following.
...
Note, however, that using the vector of parameter values in this way requires that the values are provided in the order that the function expects them. This order can be seen by displaying the function code and noting the order of the arguments in the function declaration.

```{r}
vb1
```

The same function can be used to predict mean lengths for a number of ages.
...
An equation of the growth equation with the parameter estimates can be created with `showGrowthFun()` using the same `type=` and `pname=` as before, but also including the saved `nls()` object in `fit=`. The object returned from `showGrowthFun()` and be used in `annotate()` from `ggplot2` to add the equation to the plot. Note that `parse=TRUE` is required to convert the text returned from `showGrowthFun()` into an expression, `size=` controls the size of the text, and `x=` and `y=` are used to place the equation (with trial-and-error usually).
...
ggplot(data=GrowthData1,aes(y=tlV,x=age)) +
  ...
  annotate(geom="text",
           ...
           parse=TRUE,size=4,x=10,y=50) +
  theme_bw()
```

# Second Example -- Richards

The same general process works for other growth models. For example, suppose that your data suggests an inflection point in the growth trajectory so you want to fit the second parameterization of the Richards function, which has four parameters of $L_\infty$, $k$, $t_0$, and $b$. The same general workflow is illustrated below noting that `type="Richards"` and `param=2` is used.^[Also note that the response variable was changed to `tlR` as these are lengths in `GrowthData1` that exhibited an inflection point.]

```{r}
#| fig-width: 4.5
#| fig-height: 4.5
rich1 <- makeGrowthFun(type="Richards",param=2)
rsv1 <- findGrowthStarts(tlR~age,data=GrowthData1,type="Richards",param=2,plot=TRUE)
...
ggplot(data=GrowthData1,aes(y=tlR,x=age)) +
  ...
  annotate(geom="text",
           label=showGrowthFun(type="Richards",param=2,fit=resr1),
           parse=TRUE,size=4,x=10,y=50) +
  theme_bw()
```
...
Some growth models require certain values to be set at constant values. For example, the Francis parameterization estimates three parameters ($L_1$, $L_2$, and $L_3$), which are mean lengths at three specified ages. Two of those ages -- $t_1$ and $t_3$ -- are set by the user for defining $L_1$ and $L_3$. The third parameter ($L_2$) is defined at a time that is the average of $t_1$ and $t_3$. Ages for $t_1$ and $t_3$ must be set in a named vector that is then given to `constvals=` in `findGrowthStarts()`, the function created by `makeGrowthFun()`, and `showGrowthFun()`. The rest of the workflow is the same as above (noting that `type=` and `pname=` are adjusted).^[The result from `showGrowthFun()` was saved to an object here just to reduce clutter in the plotting code.]

```{r}
#| fig-width: 4.5
#| fig-height: 4.5
vb2 <- makeGrowthFun(type="von Bertalanffy",pname="Francis")
cv2 <- c(t1=0,t3=12)
rsv2 <- findGrowthStarts(tlV~age,data=GrowthData1,type="von Bertalanffy",pname="Francis",
...
eqn <- showGrowthFun(type="von Bertalanffy",pname="Francis",
                     fit=resv2,constvals=c(t1=0,t3=12))
...
#| fig-width: 4.5
#| fig-height: 4.5
vb3 <- makeGrowthFun(type="von Bertalanffy",pname="Somers")
rsv3 <- findGrowthStarts(tlS~age,data=GrowthData2,type="von Bertalanffy",pname="Somers",
                         plot=TRUE)
...
eqn <- showGrowthFun(type="von Bertalanffy",pname="Somers",fit=resv3)
...
#| fig-width: 4.5
#| fig-height: 4.5
vb4 <- makeGrowthFun(type="von Bertalanffy",pname="Fabens")
rsv4 <- findGrowthStarts(deltaL~deltat,data=GrowthData3,
                         type="von Bertalanffy",pname="Fabens",Lmnm="tlM")
...
#| fig-height: 1
#| results: hide
eqn <- showGrowthFun(type="von Bertalanffy",pname="Fabens",fit=resv4,plot=TRUE)
