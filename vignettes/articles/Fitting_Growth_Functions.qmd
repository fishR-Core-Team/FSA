...
ggplot() +
  ...
  theme(legend.position="inside",
        legend.position.inside=c(0.82,0.5))
...
theme_grow <- function() {
  theme_bw() +
    theme(panel.grid=element_blank(),
          ...
          legend.justification.inside=c(1.1,-0.05))
}
```
...
From here one can observe that each instance of this Gompertz function has an infection point at age-3 (i.e., $t_i$) and that the "rate of growth" at that inflection point is greater as $g_i$ increases, making the growth function look more "S-shaped."
...
ggplot(data=tmp,mapping=aes(y=est_g,x=Age,color=gi)) +
  geom_line(linewidth=1) +
  labs(title=feqn) +
  theme_grow() + theme(legend.position.inside=c(1,0.6)) +
