...
|   8   | $E(L_t) = L_1+(L_3-L_1)\frac{1-r^{2\frac{t-t_1}{t_3-t_1}}}{1-r^2}$ with $r=\frac{L_3-L_2}{L_2-L_1}$|
...
| param | Equation |
|:-----:|:----------------------------------------------------|
|  10   | $E(L_t) = L_\infty\left(1-e^{-K(t-t_0)-\frac{CK}{2\pi sin(2\pi(t-t_s))}+\frac{CK}{2\pi sin(2\pi(t_0-t_s))}}\right)$ |
|  11   | $E(L_t) = L_\infty\left(1-e^{-K(t-t_0)-\frac{CK}{2\pi sin(2\pi(t-WP+0.5))}+\frac{CK}{2\pi sin(2\pi(t_0-WP+0.5))}}\right)$ |
|  12   | $E(L_t) = L_\infty(1-e^{-q})$ with $q=K'(t'-t_0) + \frac{K'(1-NGT)}{2\pi}sin(\frac{2\pi}{(1-NGT)(t'-t_s)}) - \frac{K'(1-NGT)}{2\pi}sin(\frac{2\pi}{(1-NGT)(t_0-t_s)})$ |
...
|  18   | $E(L_r-L_m)=\left[\frac{L_2g_1-L_1g_2}{g_1-g_2}-L_m\right]\left[1-\left(1+\frac{g_1-g_2}{L_1+L_2}\right)^{dt}\right]$ |
...
| param | Equation |
|:-----:|:----------------------------------------------------|
|   1   | $E(L_t) = \frac{L_\infty}{\left[1+b_1e^{-k(t-t_i)}\right]^\frac{1}{b_1}}$ |
|   2   | $E(L_t) = \frac{L_\infty}{\left(1+e^{-k(t-t_0)}\right)^{-b_2}}$ |
|   3   | $E(L_t) = L_\infty\left[1+\left(\left(\frac{L_0}{L_\infty}\right)^{1-b_3}-1\right)e^{-kt}\right]^\frac{1}{1-b_3}$ |
|   4   | $E(L_t) = L_\infty\left[1-\frac{1}{b_2}e^{-k(t-t_i)}\right]^{b_2}$ |
|   5   | $E(L_t) = L_\infty\left[1+(b_3-1)e^{-k(t-t_i)}\right]^\frac{1}{1-b_3}$ |
...
The first parameterization of the Richards growth function implemented in `FSA` is exactly the same as @eq-rSSRichards when $b_1=M$, $k=K$, and $t_i=\text{Infl}$ (and, of course, $L_\infty=\text{Asym}$).
