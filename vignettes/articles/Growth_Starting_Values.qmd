...
|  15   | $E(L_r-L_m)=(L_\infty+\beta(\bar{L}_m-L_m)-L_m)\left(1-e^{-K\delta t}\right)$ |
...
| param | Equation |
|:-----:|:----------------------------------------------------|
|   1   | $E(L_t) = L_\infty\left[1-\frac{1}{b_1}e^{-k(t-t_i)}\right]^{b}$ |
...
- $b$ = nuisance parameters with no biological meaning
...
In growth (in length) modeling, $Y$ is $L$ (for length), $X$ is $t$ (for time as measured by age), and $\text{Asym}$ is clearly $L_{\infty}$. Thus, at this point, their (modified) function can be re-written for the purposes of growth modeling as 
...
The first parameterization of the Richards growth function implemented in `FSA` is exactly the same as @eq-rSSRichards when $b=-\frac{1}{M}$, $k=K$, and $t_i=\text{Infl}$ (and, of course, $L_\infty=\text{Asym}$).

The second parameterization of the Richards growth function **NEED TO COME BACK TO THIS, BUT SOMEHOW I FOUND THAT** $t_0 = -\frac{log\left(\frac{1}{M}\right)}{K}+\text{Infl}$ and $b = -\frac{1}{M}$

Tjorve and Tjorve (2010) derived the third parameterization of the Richards growth function by **NEED TO COME BACK TO THIS, BUT SOMEHOW I FOUND THAT** $b=-\frac{1}{M}$. If $t$ is then set to $0$ in the equation for the first parameterization and solved for $L$, then $L_0=L_\infty\left[1-\frac{1}{b}e^{kt_i}\right]^{b}$. Thus, starting values for $L_0$ can be estimated from the starting values of $L_\infty$, $k$, $t_i$, and $b$ from the first parameterization.
...
plot(0,col="white",xlim=c(0,15),ylim=c(0,450),xlab="Age",ylab="Length")
curve(r1(x,Linf=Asym,k=K,ti=Infl,b1=-1/M),from=0,to=15,col="red",lwd=9,add=TRUE)
curve(r2(x,Linf=Asym,k=K,t0=-log(1/M)/K+Infl,b2=-1/M),from=0,to=15,col="blue",lwd=6,add=TRUE)
curve(r3(x,Linf=Asym,k=K,L0=Asym*((1+M*exp(K*Infl)))^(-1/M),b3=-1/M),from=0,to=15,col="green",lwd=3,add=TRUE)
```
...
# brief view of data
print(peek(df[,c("age","tlv")],n=6),row.names=FALSE)
```
...
```{r}
#| echo: true
( sv1 <- findGrowthStarts(tlv~age,data=df,type="von Bertalanffy",param=1) )
```
...
```{r}
#| echo: true
( sv1 <- findGrowthStarts(tlv~age,data=df,type="von Bertalanffy",pname="Typical") )
```
...
```{r}
#| echo: true
( sv2 <- findGrowthStarts(tlv~age,data=df,type="von Bertalanffy",param=1,
                          fixed=c("K"=0.5)) )
```
...
```{r}
#| echo: true
( sv3 <- findGrowthStarts(tlv~age,data=df,type="von Bertalanffy",param=7,
...
#| fig-width: 4.5
#| fig-height: 4.5
sv4 <- findGrowthStarts(tlv~age,data=df,type="von Bertalanffy",param=7,
...
```{r}
#| echo: true
( sv5 <- findGrowthStarts(tlv~age,data=df,type="Gompertz",param=2) )
```
...
rvb1 <- nls(tlv~vonb1(age,Linf,K,t0),data=df,start=sv1)
coef(rvb1)
```
