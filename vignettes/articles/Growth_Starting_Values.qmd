---
title: "Deriving Startig Values for Growth Functions in FSA"
author: "Derek H. Ogle"
date: "`r Sys.Date()`"
format: 
  html: 
...
In the Wang models $\beta$ is 0 if there is no individual variation in growth. In my experience a starting value for $\beta$ near 0, but no 0, is usually sufficient. `FSA` defaults to use $\beta=0.1$ as a starting value. A starting value for $\alpha$ of $L+\infty-\bar{L}_m$ is used by noting that if $\beta=0$ then parameterizations 15 and 16 are equal when $\alpha=L\infy-L_m$. An average $\alpha$ can be estimated by using $\bar{L}_m$ for $L_m$. These starting values for $\alpha$ and $\beta$ have not been rigorously tested.
