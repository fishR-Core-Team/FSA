...
test_that("metaM() messages",{
  ...
  metaM("PaulyL",Linf=200,K=0.3,T=-3) %>%
    expect_warning( "'Temp' value seems unreasonable") %>%
    suppressWarnings()
...
test_that("metaM() matches results from Kenchington (2014)",{  
  ...
  expect_equal(round(tmp$M,2)[-c(4,6)],tmp$expM[-c(4,6)])
  ...
  expect_equal(round(tmp$M,2),tmp$expM)
})
...
test_that("metaM() matches M.empirical() from fishmethods for Rio Formosa Seahorse",{
  ...
  tmp <- metaM(meths,tmax=5.5,Linf=19.76,Winf=17.3,K=0.571,t0=-0.91,
               b=3.276,t50=0.49,T=19,L=10)
  tmp2 <- fishmethods::M.empirical(Linf=19.76,Winf=17.3,Kl=0.571,Kw=0.571,TC=19,
