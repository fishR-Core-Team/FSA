...
test_that("vbStarts() output",{
  ...
  if (require(fishmethods)) {
    ...
    expect_is(tmp,"list")
    expect_equal(names(tmp),c("Linf","omega","L0"))
    ...
    tmp <- vbStarts(length~age,data=Kimura,type="Schnute",ages2use=c(1,10),meth0="yngAge")
    expect_is(tmp,"list")
    expect_equal(names(tmp),c("L1","L3","K"))
    tmp <- vbStarts(length~age,data=Kimura,type="Schnute",ages2use=c(1,10),methEV="means")
    expect_is(tmp,"list")
    expect_equal(names(tmp),c("L1","L3","K"))
    tmp <- vbStarts(length~age,data=Kimura,type="Schnute",ages2use=c(1,10),methEV="means",meth0="yngAge")
    expect_is(tmp,"list")
    expect_equal(names(tmp),c("L1","L3","K"))
    tmp <- vbStarts(length~age,data=Kimura,type="Francis",ages2use=c(1,10))
    expect_is(tmp,"list")
    expect_equal(names(tmp),c("L1","L2","L3"))
    tmp <- vbStarts(length~age,data=Kimura,type="Francis",ages2use=c(1,10),meth0="yngAge")
    expect_is(tmp,"list")
    expect_equal(names(tmp),c("L1","L2","L3"))
    tmp <- vbStarts(length~age,data=Kimura,type="Francis",ages2use=c(1,10),methEV="means")
    expect_is(tmp,"list")
    expect_equal(names(tmp),c("L1","L2","L3"))
    tmp <- vbStarts(length~age,data=Kimura,type="Francis",ages2use=c(1,10),methEV="means",meth0="yngAge")
    expect_is(tmp,"list")
    expect_equal(names(tmp),c("L1","L2","L3"))
