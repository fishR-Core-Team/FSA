...
test_that("chapmanRobson() output types",{
  ...
  # coef with as.df=TRUE
  crA <- coef(cr,as.df=TRUE)
  expect_true(is.data.frame(crA))
  expect_equal(nrow(crA),1)
  expect_equal(ncol(crA),2)
  expect_equal(names(crA),c("S","Z"))
  crA <- coef(cr,parm="S",as.df=TRUE)
  expect_true(is.data.frame(crA))
  expect_equal(nrow(crA),1)
  expect_equal(ncol(crA),1)
  expect_equal(names(crA),"S")
  # summary
  crA <- summary(cr)
  ...
  expect_equal(nrow(crA),1)
  expect_equal(ncol(crA),2)
  expect_equal(rownames(crA),"S")
  expect_equal(colnames(crA),c("Estimate","Std. Error"))
  # summary with as.df=TRUE
  crA <- summary(cr,as.df=TRUE)
  expect_true(is.data.frame(crA))
  expect_equal(nrow(crA),1)
  expect_equal(ncol(crA),4)
  expect_equal(names(crA),c("S","S_SE","Z","Z_SE"))
  crA <- summary(cr,parm="S",as.df=TRUE)
  expect_true(is.data.frame(crA))
  expect_equal(nrow(crA),1)
  expect_equal(ncol(crA),2)
  expect_equal(names(crA),c("S","S_SE"))
  # confint
  crA <- confint(cr)
  expect_equal(class(crA),c("matrix","array"))
  expect_equal(mode(crA),"numeric")
  ...
  expect_equal(rownames(crA),"S")
  expect_equal(colnames(crA),c("95% LCI","95% UCI"))
  # confint with incl.est=TRUE
  crA <- confint(cr,incl.est=TRUE)
  expect_equal(class(crA),c("matrix","array"))
  expect_equal(mode(crA),"numeric")
  expect_equal(nrow(crA),2)
  expect_equal(ncol(crA),3)
  expect_equal(rownames(crA),c("S","Z"))
  expect_equal(colnames(crA),c("Est","95% LCI","95% UCI"))
  crA <- confint(cr,parm="S",incl.est=TRUE)
  expect_equal(class(crA),c("matrix","array"))
  expect_equal(mode(crA),"numeric")
  expect_equal(nrow(crA),1)
  expect_equal(ncol(crA),3)
  expect_equal(rownames(crA),"S")
  expect_equal(colnames(crA),c("Est","95% LCI","95% UCI"))
  # confint with incl.est=TRUE and as.df=TRUE
  crA <- confint(cr,incl.est=TRUE,as.df=TRUE)
  expect_true(is.data.frame(crA))
  expect_equal(nrow(crA),1)
  expect_equal(ncol(crA),6)
  expect_equal(names(crA),c("S","S_LCI","S_UCI","Z","Z_LCI","Z_UCI"))
  crA <- confint(cr,parm="S",incl.est=TRUE,as.df=TRUE)
  expect_true(is.data.frame(crA))
  expect_equal(nrow(crA),1)
  expect_equal(ncol(crA),3)
  expect_equal(names(crA),c("S","S_LCI","S_UCI"))
