...
test_that("Summarize() results, quantitative variable by single factor",{
  ## no NAs or zeros
  exp <- data.frame(f1=factor(c("A","B","C")),n=rep(3,3),nvalid=rep(3,3),
  ...
  ## NAs and zeros
  exp <- data.frame(f1=factor(c("A","B","C")),n=rep(3,3),nvalid=c(2,3,3),
...
test_that("Summarize() results, quantitative variable by two factors",{
  exp <- data.frame(f2=factor(rep(c("a","b","c"),3)),
...
test_that("Summarize() results using exclude=",{
  exp <- data.frame(f1=factor(c("B","C")),n=c(3,3),nvalid=c(3,3),
...
test_that("Summarize() results, assure 'by' variable is the expected mode/type",{
  expect_equal(class(Summarize(q1~f1,data=d1)$f1),"factor")
  expect_equal(class(Summarize(q1~c1,data=d1)$c1),"character")
  expect_equal(class(Summarize(q1~q3,data=d1)$q3),"numeric")
  tmp <- Summarize(q1~f1+f2,data=d1)
  expect_equal(class(tmp$f1),"factor")
  expect_equal(class(tmp$f2),"factor")
  tmp <- Summarize(q1~c1+f1,data=d1)
  expect_equal(class(tmp$c1),"character")
  expect_equal(class(tmp$f1),"factor")
  tmp <- Summarize(q1~c1+q3,data=d1)
  expect_equal(class(tmp$c1),"character")
  expect_equal(class(tmp$q3),"numeric")
