...
test_that("capHistSum() results",{
  ...
  expect_equal(colnames(ch$methodB.top),paste0("i=",1:3))
  expect_equal(rownames(ch$methodB.top),paste0("j=",1:3))
  expect_is(ch$methodB.bot,"matrix")
  expect_equal(nrow(ch$methodB.bot),4)
  expect_equal(ncol(ch$methodB.bot),3)
  expect_equal(colnames(ch$methodB.bot),paste0("i=",1:3))
  ...
  expect_equal(rownames(ch$m.array),paste0("i=",1:3))
...
test_that("capHistConvert() results",{
  ...
  expect_equal(names(ex1.M2I),paste0("event",1:length(unique(ex1$yr))))
  # MARK format (with id)
  ...
  expect_equal(names(ex1.M2I2),c("id",paste0("event",1:length(unique(ex1$yr)))))
  # RMARK format
  ...
  expect_equal(names(ex1.R2I),c("fish",paste0("event",1:length(unique(ex1$yr)))))
  ...
  expect_equal(names(ex1.M2F),c(paste0("event",1:length(unique(ex1$yr))),"freq"))
  # RMark format
  ...
  expect_equal(names(ex1.R2F),c(paste0("event",1:length(unique(ex1$yr))),"freq"))
