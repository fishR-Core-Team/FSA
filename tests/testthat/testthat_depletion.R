...
test_that("depletion() output types",{
  ...
  expect_equal(class(l1A),"numeric")
  expect_equal(length(l1A),2)
  expect_equal(names(l1A),c("No","q"))
  l1A <- coef(l1,as.df=TRUE)
  expect_true(is.data.frame(l1A))
  ...
  l1A <- coef(l1,parm="No",as.df=TRUE)
  expect_true(is.data.frame(l1A))
  expect_equal(ncol(l1A),1)
  expect_equal(nrow(l1A),1)
  expect_equal(names(l1A),"No")
  l1A <- coef(l1,parm="lm")
  expect_true(is.vector(l1A))
  expect_equal(class(l1A),"numeric")
  expect_equal(length(l1A),2)
  expect_equal(names(l1A),c("(Intercept)","K"))
  l1A <- coef(l1,parm="lm",as.df=TRUE)
  expect_true(is.data.frame(l1A))
  expect_equal(ncol(l1A),2)
  expect_equal(nrow(l1A),1)
  expect_equal(names(l1A),c("Intercept","K"))
  ...
  l1A <- confint(l1,as.df=TRUE)
  expect_true(is.data.frame(l1A))
  expect_equal(ncol(l1A),4)
  expect_equal(nrow(l1A),1)
  expect_equal(names(l1A),c("No_LCI","No_UCI","q_LCI","q_UCI"))
  ...
  expect_equal(colnames(l1A),c("95% LCI","95% UCI"))
  l1A <- confint(l1,parm="No",as.df=TRUE)
  expect_true(is.data.frame(l1A))
  expect_equal(ncol(l1A),2)
  expect_equal(nrow(l1A),1)
  expect_equal(names(l1A),c("No_LCI","No_UCI"))
  ...
  expect_equal(colnames(l1A),c("95% LCI","95% UCI"))
  l1A <- confint(l1,parm="lm",as.df=TRUE)
  expect_true(is.data.frame(l1A))
  expect_equal(ncol(l1A),4)
  expect_equal(nrow(l1A),1)
  expect_equal(names(l1A),c("(Intercept)_LCI","(Intercept)_UCI","K_LCI","K_UCI"))
  # confint()s with incl.est=TRUE
  l1A <- confint(l1,incl.est=TRUE)
  expect_true(is.matrix(l1A))
  expect_equal(mode(l1A),"numeric")
  expect_equal(nrow(l1A),2)
  expect_equal(ncol(l1A),3)
  expect_equal(rownames(l1A),c("No","q"))
  expect_equal(colnames(l1A),c("Est","95% LCI","95% UCI"))
  l1A <- confint(l1,as.df=TRUE,incl.est=TRUE)
  expect_true(is.data.frame(l1A))
  expect_equal(ncol(l1A),6)
  expect_equal(nrow(l1A),1)
  expect_equal(names(l1A),c("No","No_LCI","No_UCI","q","q_LCI","q_UCI"))
  l1A <- confint(l1,parm="No",incl.est=TRUE)
  expect_true(is.matrix(l1A))
  expect_equal(mode(l1A),"numeric")
  expect_equal(ncol(l1A),3)
  expect_equal(nrow(l1A),1)
  expect_equal(rownames(l1A),c("No"))
  expect_equal(colnames(l1A),c("Est","95% LCI","95% UCI"))
  l1A <- confint(l1,parm="No",as.df=TRUE,incl.est=TRUE)
  expect_true(is.data.frame(l1A))
  expect_equal(ncol(l1A),3)
  expect_equal(nrow(l1A),1)
  expect_equal(names(l1A),c("No","No_LCI","No_UCI"))
  l1A <- confint(l1,parm="lm",incl.est=TRUE)
  expect_true(is.matrix(l1A))
  expect_equal(mode(l1A),"numeric")
  expect_equal(nrow(l1A),2)
  expect_equal(ncol(l1A),3)
  expect_equal(rownames(l1A),c("(Intercept)","K"))
  expect_equal(colnames(l1A),c("Est","95% LCI","95% UCI"))
  l1A <- confint(l1,parm="lm",as.df=TRUE,incl.est=TRUE)
  expect_true(is.data.frame(l1A))
  expect_equal(ncol(l1A),6)
  expect_equal(nrow(l1A),1)
  expect_equal(names(l1A),c("(Intercept)","(Intercept)_LCI","(Intercept)_UCI",
                            "K","K_LCI","K_UCI"))
  # summary()s
  l1A <- summary(l1)
  expect_true(is.matrix(l1A))
  ...
  l1A <- summary(l1,as.df=TRUE)
  expect_true(is.data.frame(l1A))
  expect_equal(ncol(l1A),4)
  expect_equal(nrow(l1A),1)
  expect_equal(names(l1A),c("No_Est","No_SE","q_Est","q_SE"))
  ...
  expect_equal(rownames(l1A),c("No"))
  expect_equal(colnames(l1A),c("Estimate","Std. Err."))
  l1A <- summary(l1,parm="No",as.df=TRUE)
  expect_true(is.data.frame(l1A))
  expect_equal(ncol(l1A),2)
  expect_equal(nrow(l1A),1)
  expect_equal(names(l1A),c("No_Est","No_SE"))
  ...
  ## did not check as.df=TRUE with method="DeLury"
  d1 <- depletion(SMBassLS$catch,SMBassLS$effort,method="DeLury")
  expect_equal(class(d1),"depletion")
  # coef()s
  d1A <- coef(d1)
  expect_true(is.vector(d1A))
  expect_equal(class(d1A),"numeric")
...
test_that("depletion() with 'Leslie' matches fishmethod's 'deplet' for darter data",{
  ...
  expect_equal(round(cf1["q",],7),round(cf1fm["q",],7),ignore_attr=TRUE)
  expect_equal(round(ci1["No",],1),round(ci1fm["N",],1))
  expect_equal(round(ci1["q",],6),round(ci1fm["q",],6))
})
...
test_that("depletion() with 'Leslie' matches fishmethod's 'deplet' for lobster data",{
  ...
  expect_equal(round(cf3["q",],7),round(cf3fm["q",],7),ignore_attr=TRUE)
  expect_equal(round(ci3["No",],1),round(ci3fm["N",],1))
  expect_equal(round(ci3["q",],5),round(ci3fm["q",],5))
})
...
test_that("depletion() with 'Leslie' matches fishmethod's 'deplet' for blue crab data",{
  ...
  expect_equal(round(cf5["q",],7),round(cf5fm["q",],7),ignore_attr=TRUE)
  expect_equal(round(ci5["No",],1),round(ci5fm["N",],1))
  expect_equal(round(ci5["q",],6),round(ci5fm["q",],6))
})
...
test_that("depletion() with 'Leslie' matches fishmethod's 'deplet' for SMB data",{
  ...
  expect_equal(round(cf7["q",],7),round(cf7fm["q",],7),ignore_attr=TRUE)
  expect_equal(round(ci7["No",],1),round(ci7fm["N",],1))
  expect_equal(round(ci7["q",],5),round(ci7fm["q",],5))
})
...
test_that("depletion() with 'DeLury' and Ricker.mod matches fishmethod's 'deplet'",{
  ...
  expect_equal(round(cf2["q",],7),round(cf2fm["q",],7),ignore_attr=TRUE)
  expect_equal(round(ci2["No",],1),round(ci2fm["N",],1))
  expect_equal(round(ci2["q",],6),round(ci2fm["q",],6))
})
...
test_that("depletion() with 'DeLury' and Ricker.mod matches fishmethod's 'deplet' for the lobster data",{
  ...
  expect_equal(round(cf4["q",],7),round(cf4fm["q",],7),ignore_attr=TRUE)
  expect_equal(round(ci4["No",],1),round(ci4fm["N",],1))
  expect_equal(round(ci4["q",],7),round(ci4fm["q",],7))
})
...
test_that("depletion() with 'DeLury' and Ricker.mod matches fishmethod's 'deplet' for the blue crab data",{
  ...
  expect_equal(round(cf6["q",],7),round(cf6fm["q",],7),ignore_attr=TRUE)
  expect_equal(round(ci6["No",],1),round(ci6fm["N",],1))
  expect_equal(round(ci6["q",],5),round(ci6fm["q",],5))
})
...
test_that("depletion() with 'DeLury' and Ricker.mod matches fishmethod's 'deplet' for the SMB data",{
  ...
  expect_equal(round(cf8["q",],7),round(cf8fm["q",],7),ignore_attr=TRUE)
  expect_equal(round(ci8["No",],1),round(ci8fm["N",],1))
  expect_equal(round(ci8["q",],6),round(ci8fm["q",],6))
})
