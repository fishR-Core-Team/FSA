# FSA 0.4.39 ongoing
* `catchCurve()`: Modified.  Changed `use.weights=` to `weighted=`.  Added some checks for the formula in the formula version and for the variables in the default version.  Add unit tests for warnings and errors and one test for values.
* `chapmanRobson()`: Modified.  Added some checks for the formula in the formula version and for the variables in the default version.  Added `verbose=` to `summary()`.  Add unit tests for warnings and errors and one test for values.
